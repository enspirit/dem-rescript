...
let fco_of_option_filename = o => o->Belt.Option.map(f => Fsi.Filename(f))
...
let copts_t = {
  ...
  let template_filename = {
    ...
    \"&"(
      ...
      \"&"(
        ...
        info(list{"h", "html-template"}, ~docv, ~doc),
      )
    )
  }
  let style_filename = {
    ...
    open Cmdliner.Arg
    \"&"(value, \"&"(opt(some(string), None), info(list{"s", "style"}, ~docv, ~doc)))
  }
  let text_filename = {
    let docv = "FILE"
    ...
    open Cmdliner.Arg
    \"&"(value, \"&"(opt(some(string), Some("index.md")), info(list{"t", "text"}, ~docv, ~doc)))
  }
  let data_filename = {
    let docv = "FILE"
    ...
    open Cmdliner.Arg
    \"&"(value, \"&"(opt(some(string), None), info(list{"d", "data"}, ~docv, ~doc)))
  }
  let watch_mode = {
    ...
    open Cmdliner.Arg
    \"&"(value, \"&"(flag, info(list{"w", "watch"}, ~doc)))
  }
  let base_url = {
    let docv = "URL"
    ...
    open Cmdliner.Arg
    \"&"(value, \"&"(opt(some(string), None), info(list{"u", "base-url"}, ~docv, ~doc)))
  }
  let output_filename = {
    let docv = "FILE"
  ...
  let publipost = {
    ...
    open Cmdliner.Arg
    \"&"(value, \"&"(flag, info(list{"publipost"}, ~doc)))
  }
  let async = {
    ...
    open Cmdliner.Arg
    \"&"(value, \"&"(flag, info(list{"async"}, ~doc)))
  }
  open Cmdliner.Term
  \"$"(
    \"$"(
      \"$"(
        \"$"(
          \"$"(
            \"$"(
              \"$"(\"$"(\"$"(const(copts), template_filename), style_filename), text_filename),
              data_filename,
            ),
            watch_mode,
          ),
...
let help = (_, man_format, cmds, topic) =>
  ...
  | Some(topic) =>
    let topics = list{"topics", "patterns", "environment", ...cmds}
    ...
    | #Ok(_) =>
      let page = ((topic, 7, "", "", ""), list{#S(topic), #P("Say something")})
...
let instantiate_cmd = {
  ...
  let man = list{
    ...
    #P(
      "Instantiates documents written in Markdown with business data injected from
  ...
  (
    {
      open Cmdliner.Term
      ret(\"$"(const(Fsi.instantiate |> copts_t_of_fsi_result), copts_t))
    },
...
let compile_cmd = {
  ...
  let man = list{
    #S(Cmdliner.Manpage.s_description),
    #P(
      "Compiles documents written in Markdown, styled in CSS, with business data injected from
  ...
  (
    {
      open Cmdliner.Term
      ret(\"$"(const(Fsi.compile |> copts_t_of_fsi_result), copts_t))
    },
...
let print_cmd = {
  ...
  let man = list{
    #S(Cmdliner.Manpage.s_description),
    #P(
      "Prints a PDF document after compiling a doc-e-mate source written with Markdown, styled in CSS, with
  ...
  (
    {
      open Cmdliner.Term
      ret(\"$"(\"$"(const(Fsi.print |> copts_t_of_fsi_print_result), copts_t), pipe))
    },
...
let help_cmd = {
  let topic = {
    ...
    open Cmdliner.Arg
    \"&"(value, \"&"(pos(0, some(string), None), info(list{}, ~docv="TOPIC", ~doc)))
  }
  ...
  (
    {
      ...
      ret(
        \"$"(
          \"$"(
            \"$"(\"$"(const(help), copts_t), Cmdliner.Arg.man_format),
            Cmdliner.Term.choice_names,
          ),
          topic,
        ),
      )
...
let default_cmd = {
  ...
  (
    ...
    Cmdliner.Term.info("dem", ~version, ~doc, ~sdocs, ~exits, ~man),
  )
}

// Available legal commands.
let cmds = list{help_cmd, instantiate_cmd, compile_cmd, print_cmd}
